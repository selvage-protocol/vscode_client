...
/**
 * A link this subject accepted without seating anything: the fragment guard removed by a mutation
 * let through a join it refuses, and a refusal has no frame to report from. Held so the `join`
 * after it is not refused for a session that was never there.
 */
let clear = false;

/**
 * The guards on the link (`runner/subject.py`'s `LINK_MUTATIONS`), removed before the `join` that
 * reads it: §5.1's fragment is decided before a session exists, so the removal is held here until
 * the next `join` and not handed to a session that is not there yet.
 */
const LINK_MUTATIONS = new Set(['accept-partial-fragment']);
